...
namespace MVCQLBH.Controllers
{
    ...
    public class ManageProductController : Controller
    {
        ...
        public ActionResult AddNSX(NSXInfo nsx)
        {
            using (var dc = new QLBHEntities())
            {
                var nsxTonTai = dc.NhaSanXuats.Where(m => m.TenNhaSanXuat.Contains(nsx.NSXNameInfo)).FirstOrDefault();
                if (nsxTonTai != null)
                {
        ...
        public ActionResult UpdateInfoCat(CatInfo cat)
        {
            using (var dc = new QLBHEntities())
            {
                ...
                if (cU != null)
                {
                    ...
                    var a = dc.Categories.Where(c => c.CatName.Contains(cU.CatName)).FirstOrDefault();
                    if(a != null)
                    {
                        ViewBag.ErrorMsg = "Tên loại sản phẩm đã tồn tại";
                    }
        ...
        public ActionResult UpdateInfoNSX(NSXInfo nsx)
        {
            using (var dc = new QLBHEntities())
            {
                // Kiểm tra ID nhà sản xuất nhập vào có tồn tại không
                var nsxU = dc.NhaSanXuats.Where(c => c.IDNhaSanXuat == nsx.NSXIDInfo).FirstOrDefault();
                if (nsxU != null)
                {
                    nsxU.TenNhaSanXuat = nsx.NSXNameInfo;

                    // Kiểm tra tên nhà sản xuât nhập vào có bị trùng không
                    var a = dc.NhaSanXuats.Where(c => c.TenNhaSanXuat.Contains(nsxU.TenNhaSanXuat)).FirstOrDefault();
                    if (a != null)
                    {
                        ViewBag.ErrorMsg = "Tên nhà sản xuất đã tồn tại";
                    }
