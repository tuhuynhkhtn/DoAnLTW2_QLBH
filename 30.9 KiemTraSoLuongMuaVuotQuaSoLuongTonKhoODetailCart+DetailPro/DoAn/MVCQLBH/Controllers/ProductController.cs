...
namespace MVCQLBH.Controllers
{
    public class ProductController : Controller
    {
        ...
        ////Số lượng sản phẩm tối đa hiển thị trên 1 trang
        ////GET: Product
        ...
        public ActionResult AjaxListByCat(int? id, int page = 1)
        {
            ...
            using (var dc = new QLBHEntities())
            {
                ...
                var l = dc.Products
                    ...
                    .Skip((page - 1) * nPerPage)
                    .Take(nPerPage)
                    .Select(p => new { ProID = p.ProID, ProName = p.ProName, Price = p.Price, TinyDes = p.TinyDes, Page = page, Quantity = p.Quantity})
                    .ToList();
                //return View("ListByCategory", l);
        ...
        public ActionResult GetListByNSX(int? id, int page = 1)
        {
            ...
            using (var dc = new QLBHEntities())
            {
                ...
                var l = dc.Products
                    ...
                    .Take(nPerPage)
                    .ToList();
                return View("AjaxListByNSX", l);
            }
        }

        //Ajax NSX
        public ActionResult AjaxListByNSX(int? id, int page = 1)
        {
            if (!id.HasValue)
            {
                //return RedirectToAction("Index", "Home");
        ...
        public ActionResult DetailPro(int? id, int errorCart = 1)
        {
            if (errorCart == 0)
            {
                ViewBag.ErrorQ = 1;
        ...
        // GET: Product
        public ActionResult HienThi5SPCungLoai(int loaiSP, int idPro)
        {
            using (var dc = new QLBHEntities())
            {
                var l = dc.Products
