...
namespace MVCQLBH.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(UserInfo ui)
        {
            ...
            using (var dc = new QLBHEntities())
            {
                ...
                if (user != null)
                {
                    ...
                    //Kiem tra thong tin dang nhap co fai la admin hay khong
                    //neu fai thi tra ve admin khong can chay xuong duoi

                    //Admin
                    if(ui.Permission == 1)
                    {
                        return RedirectToAction("IndexManageProduct", "ManageProduct");
                    }
        ...
        public ActionResult AjaxRegister(UserRegisting ur)
        {
            ...
            else
            {
                ...
                var u = new User
                {
                    ...
                    f_Name = ur.Name,
                    f_Email = ur.Email,
                    f_DOB = DateTime.ParseExact(ur.DOB, "d/M/yyyy", null)
                };
                using (var dc = new QLBHEntities())
                {
                    dc.Users.Add(u);
        ...
        public ActionResult UpdateProfile(UserInfo user)
        {
            using (var dc = new QLBHEntities())
            {
                ...
                if (u != null)
                {
                    ...
                    u.f_DOB = DateTime.ParseExact(user.DOB, "dd/MM/yyyy", null);
        ...
        public ActionResult AjaxProfile()
        {
            ...
            using (var dc = new QLBHEntities())
            {
                var u = dc.Users.Where(m => m.f_Username == ui.Username).FirstOrDefault();
                if (u != null)
                {
                    var a = new UserInfo
                    {
        ...
        public ActionResult AjaxUpdateProfile(UserInfo user)
        {
            ...
            else
            {
                using (var dc = new QLBHEntities())
                {
                    var us = dc.Users.Where(u => u.f_ID == user.UserID).FirstOrDefault();
                    if(us != null)
                    {
                        string d = null;
                        if(user.DOB.Length > 10)
                        {
                            d = user.DOB.Remove(10);
                        }
                        else
                        {
                            d = user.DOB;
                        ...
                        us.f_Username = user.Username;
                        us.f_Name = user.Name;
                        us.f_Email = user.Email;
                        us.f_DOB = DateTime.ParseExact(user.DOB, "d/M/yyyy", null);
        ...
        [AuthActionFilter]
        public ActionResult AjaxPassword()
        {
            var ui = Session["Logged"] as UserInfo;
            using (var dc = new QLBHEntities())
            {
        ...
        [HttpPost]
        public ActionResult AjaxPassword(UserInfo user)
        {
            PasswordError l = new PasswordError();
            l = AddHelpers.GetErrorNullPassword(user);

            //No Error
            if (l.ErrorPasswordOld != null || l.ErrorPasswordNew != null || l.ErrorPasswordRetype != null)
            {
                Thread.Sleep(1000);
                return Json(l);
            }
            ...
            {
                string passNew = Ulti.Md5Hash(user.PasswordNew);
                using (var dc = new QLBHEntities())
                {
                    var us = dc.Users.Where(u => u.f_ID == user.UserID).FirstOrDefault();
                    us.f_Password = passNew;
                    dc.SaveChanges();
                    Thread.Sleep(1000);
                    return Json(l);
                }
