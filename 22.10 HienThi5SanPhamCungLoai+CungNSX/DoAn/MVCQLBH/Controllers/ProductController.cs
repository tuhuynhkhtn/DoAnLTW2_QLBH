...
namespace MVCQLBH.Controllers
{
    public class ProductController : Controller
    {
        ...
        // GET: Home
        public ActionResult HienThi5SPCungLoai(string loaiSP)
        {
            using (var dc = new QLBHEntities())
            {
                var l = dc.Products
                    .Where(p => p.BiXoa == 0 && p.Loai == loaiSP)
                    .Take(5)
                    .ToList();
        ...
        // GET: Home
        public ActionResult HienThi5SPCungNSX(int nsx)
        {
            using (var dc = new QLBHEntities())
            {
                var l = dc.Products
                    .Where(p => p.BiXoa == 0 && p.IDNhaSanXuat == nsx)
                    .Take(5)
                    .ToList();
                return PartialView("_PartialHienThi5SP", l);
            }
